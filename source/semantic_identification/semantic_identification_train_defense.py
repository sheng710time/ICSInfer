...
def train_file(protocol, stsm_type, model_file_name, region, excluded_operations, defense_mode):
    ...
    # defense methods
    train_device_packets = None
    if defense_mode=="pad":
        train_defense_packets = defense_padding_packets(train_filtered_packets, protocol)
        train_device_packets = split_packets_by_ip(train_defense_packets, protocol)
    elif defense_mode=="encry":
        train_defense_packets = defense_encryption_packets(train_filtered_packets, protocol)
        train_device_packets = split_packets_by_ip(train_defense_packets, protocol)
    elif defense_mode=="noise":
        train_defense_packets = defense_noise_packets(train_filtered_packets, protocol)
        train_device_packets = split_packets_by_ip(train_defense_packets, protocol)
    elif defense_mode == "remove":
        train_defense_packets = remove_response_packets(train_filtered_packets, protocol)
        train_device_packets = split_packets_by_ip(train_defense_packets, protocol)
    elif defense_mode == "diff":
        train_defense_packets = defense_different_encryption_packets(train_filtered_packets, protocol)
        train_device_packets = split_packets_by_ip(train_defense_packets, protocol)
    else:
...
if __name__ == '__main__':
    ...
    start_file_index = 41  # The index of the first training PCAP file
    ...
    defense_mode = "diff"  # pad, encry, noise, remove, diff
    ...
    excluded_operations = []
    train_file(protocol, stsm_type, model_file_name, region, excluded_operations, defense_mode)
