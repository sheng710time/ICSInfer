...
def get_train_ip_list_file(protocol):
    require_supported_protocol(protocol)
    return protocol_device_info_file(protocol)


def get_train_valid_ips(protocol):
    train_ip_list_file = get_train_ip_list_file(protocol)
    require_existing_file(train_ip_list_file, f"{protocol} device information CSV")
    train_valid_ips = get_ip_list(train_ip_list_file)
    print(f"Train candidate IPs from scan_valid: {len(train_valid_ips)}")
    return train_valid_ips


def train_file(protocol, stsm_type, model_file_name, region, excluded_operations):
    train_valid_ips = get_train_valid_ips(protocol)
    train_all_packets = []
    ...
    for pcap_file in pcap_files:
        ...
        train_all_packets.extend(get_packets_inlist_others(pcap_path, train_valid_ips))
